...
struct Hospitals {
	 ...
	 float BedsPrice; //per night (surgery)
	 float HospitalRate;
...
void DefinitonOfVariable(Hospitals hospital[], int& hospitalCount) {
	 ...
	 for (int i = 0; !HospitalInfo.eof(); i++) {
		  ...
		  HospitalInfo >> hospital[i].HospitalID >> hospital[i].PatientReservationRooms >> hospital[i].ReservationPrice >> hospital[i].BedsPrice >> hospital[i].HospitalRate >> hospital[i].specialtiesCount >> hospital[i].clinicsCount>>hospital[i].surgeryprice;
...
void SaveData(Hospitals hospital[], int hospitalCount) {
	 ...
	 for (int i = 0; i < hospitalCount; i++) {
		  ExportHospitalInfo << hospital[i].HospitalName << "* ";
